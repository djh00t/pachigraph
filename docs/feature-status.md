...
| Native D1 schema and empty-store baseline | 🟢 Live verified | Sites database overview and bounded row reads | Obtain physical size and capacity metrics before sample growth measurement | Orchestrator (no subagent) | current session | not exposed |
